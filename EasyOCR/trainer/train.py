...
def train(opt, show_number = 2, amp=False):
    ...
    while(True):
        ...
        if (i % opt.valInterval == 0) and (i!=0):
            ...
            with open(f'./saved_models/{opt.experiment_name}/log_train.txt', 'a', encoding="utf8") as log:
                ...
                if current_accuracy > best_accuracy:
                    ...
                    torch.save(model.state_dict(), fr'{opt.path_save_model}/{opt.experiment_name}/best_accuracy.pth')
                if current_norm_ED > best_norm_ED:
                    best_norm_ED = current_norm_ED
                    torch.save(model.state_dict(), fr'{opt.path_save_model}/{opt.experiment_name}/best_norm_ED.pth')
        ...
        if (i + 1) % 1e+4 == 0:
            torch.save(
                model.state_dict(), fr'{opt.path_save_model}/{opt.experiment_name}/iter_{i+1}.pth')
