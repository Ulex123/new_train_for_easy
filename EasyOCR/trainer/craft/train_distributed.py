...
class Trainer(object):
    ...
    def train(self, buffer_dict):
        ...
        if self.config.mode == "weak_supervision":
            ...
            # NOTE: only work on half GPU assign recognition_module / half GPU assign supervision setting
...
def main():
    parser = argparse.ArgumentParser(description="CRAFT custom data recognition_module")
    parser.add_argument(
        "--yaml",
        "--yaml_file_name",
    ...
    if config["mode"] == "weak_supervision":
        # NOTE: half GPU assign recognition_module / half GPU assign supervision setting
...
def main_worker(gpu, port, ngpus_per_node, config, buffer_dict, exp_name, mode):
    ...
    batch_size = int(config["recognition_module"]["batch_size"] / ngpus_per_node)
    config["recognition_module"]["batch_size"] = batch_size
    config = DotDict(config)

    # Start recognition_module
